			...
			if (srd.accel_count >= 0)
			{
				if (is_stopping)
				{
					...
					if (srd.t_pause == 0) {
						...
						// Выставим величину, с которой мы сравниваем на прерывания каждую секунду
						OCR1A = 62500 - 1; // НЕТ, ЭТО НЕ ОШИБКА, ЭТО НЕ 16 бит, это 16MHz/256-1
